...
static void publish_state(bool on) {
    if (s_client) {
        esp_mqtt_client_publish(s_client, topic_state, on ? "ON" : "OFF", 0, 1, true);
    }
}
...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        if (event->current_data_offset != 0 ||
            ...
            ESP_LOGW(TAG, "ignoring fragmented payload (offset=%d len=%d total=%d)",
        ...
        if (event->topic_len == strlen(topic_cmd) &&
            ...
            if (handle_command(event->data_len, event->data) != 0) {
                ESP_LOGI(TAG, "Invalid command=%.*s", event->data_len, event->data);
            }
        }
        break;
